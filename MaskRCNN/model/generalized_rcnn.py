...
class DetectionModel(ModelDesc):
    ...
    def build_graph(self, *inputs):
        ...
        head_losses = None
        if cfg.TRAIN.PER_IMAGE_ROI_HEAD: head_losses = self.roi_heads_per_image(image, features, proposal_boxes, targets, inputs)
        else: head_losses = self.roi_heads(image, features, proposal_boxes, targets, inputs)
...
class ResNetFPNModel(DetectionModel):
    ...
    def roi_heads_per_image(self, image, features, proposal_boxes, targets, inputs):
        """
        Implement the RoI Align and construct the RoI head (box and mask branches) of the graph
        ...
        if self.training:
            ...
            if cfg.MODE_MASK:
                ...
                for i in range(cfg.TRAIN.BATCH_SIZE_PER_GPU):
                    ...
                    for j in range(4):
                        single_image_feature.append(tf.gather(features[j], i))
                    ...
                    single_image_mask_logits = maskrcnn_head_func(
                            'maskrcnn_{}'.format(i), single_image_roi_feature_maskrcnn, cfg.DATA.NUM_CATEGORY, fp16=self.fp16)
                    ...
                    single_image_target_masks_for_fg = crop_and_resize(single_image_gt_masks,
                                                                       ...
                                                                       pad_border=False,
                                                                       verbose_batch_index=i)
                    single_image_target_masks_for_fg = tf.squeeze(single_image_target_masks_for_fg, 1, 'single_image_target_masks_for_fg_{}'.format(i))
                    single_image_mask_loss = maskrcnn_loss(single_image_mask_logits, single_image_fg_labels, single_image_target_masks_for_fg)
                    all_mask_losses.append(single_image_mask_loss)

                mask_loss = tf.truediv(tf.add_n(all_mask_losses), tf.cast(cfg.TRAIN.BATCH_SIZE_PER_GPU, dtype=tf.float32), name='avg_mask_loss')
                all_losses.append(mask_loss)
            return all_losses
        else:
